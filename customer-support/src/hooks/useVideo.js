...
import { VIDEO_WINDOW } from "../constants";
...
const useVideo = ()=>{
    ...
    useEffect(() => {
        if(state.botStepper === VIDEO_WINDOW )
    ...
    const messageByBot = (msg) =>{
      ...
      const sendQuery = (msg) =>{
        ...
        setMessages((prevChat)=>([ ...prevChat, newChat]));
        axios.post('http://127.0.0.1:8000/api/getQueryVideo',{
            message:msg
        },  {
            headers: {
        ...
        .then(response=>{
            ...
            if(response.data.success){
                messageByBot(response.data.data.text);
                setSrc(response.data.data.url);
    ...
    return([messages,transcript,src])
}
